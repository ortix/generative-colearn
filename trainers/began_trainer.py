...
class BEGANTrainer(AbstractTrainer):
    ...
    def train(self,
        ...
        self.load.set_training_data(data)
        self.load.set_training_labels(labels)
        ...
        for it in tqdm(range(epochs)):
            for _ in range(d_steps):
                X_mb, y_mb = self.load.next_batch(batch_size)
                D_loss = self.network.train_discriminator(X_mb, y_mb, k)

            # X_mb, y_mb = self.load.next_batch(batch_size)
            G_loss = self.network.train_generator(y_mb)

            k = k + lam * (gamma*D_loss - G_loss)
            ...
            if it % 1000 == 0:
                n = 1000
                X_mb, y_mb = self.load.next_batch(n)
