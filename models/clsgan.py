...
class CLSGAN:
    def __init__(
        self, input_size, label_size, latent_size, layers, lr=1e-3, beta1=0.5, log=True
    ):
        ...
        self.data = tf.placeholder(
            tf.float64, shape=[None, self.input_size], name="data"
        )
        self.labels = tf.placeholder(
            tf.float64, shape=[None, self.label_size], name="labels"
        )
        ...
        # LSGAN Loss
        self.D_loss = 0.5 * (
            tf.reduce_mean(tf.square((D_real - 1))) + tf.reduce_mean(tf.square(D_fake))
        )
        ...
        self.D_solver = tf.train.AdamOptimizer(learning_rate=lr, beta1=beta1).minimize(
            self.D_loss, var_list=D_vars
        )
        self.G_solver = tf.train.AdamOptimizer(learning_rate=lr, beta1=beta1).minimize(
        ...
        self.logdir = "./logs/tensorboard/{}/".format(
    ...
    def train_discriminator(self, Z_mb, X_mb, y_mb):
        _, loss = self._sess.run(
            ...
            feed_dict={self.data: X_mb, self.noise: Z_mb, self.labels: y_mb},
        )
        return loss
    ...
    def train_generator(self, Z_mb, y_mb):
        _, loss = self._sess.run(
            ...
            feed_dict={self.noise: Z_mb, self.labels: y_mb},
        )
        return loss
    ...
    def build_discriminator(self, x, y, reuse=False):
        ...
        with tf.variable_scope(
            "discriminator", reuse=reuse, initializer=self.initializer
        ):
    ...
    def discriminate(self, labels):
        ...
        return self._sess.run(
            self.D_sample, feed_dict={self.noise: noise, self.labels: labels}
        )
    ...
    def predict(self, labels):
        ...
        return self._sess.run(
            self.G_sample, feed_dict={self.noise: noise, self.labels: labels}
        )

    def sample_z(self, n):
        return np.random.uniform(-1.0, 1, size=[n, self.latent_size])
    ...
    def save_weights(self, path):
        print("Saving weights at {}".format(path))
        self._saver.save(self._sess, path)

    def restore_weights(self, path):
        print("Restoring weights from {}".format(path))
...
if __name__ == "__main__":
    gan = CLSGAN(
        8, 12, 64, {"g": [32, 64, 128, 256, 512], "d": [32, 64, 128, 256, 512]}
    )
